...
- `pnpm dev` launches the local dev server with Contentlayer watching.
- `pnpm build` runs the production Next.js build and invokes `scripts/postbuild.mjs` to refresh feeds and sitemaps.
- `pnpm start` serves the optimized build.
- `pnpm lint` applies ESLint checks across `pages`, `app`, `components`, `layouts`, `lib`, and `scripts`.
...
The repo currently relies on linting and manual verification. Always run `pnpm lint` and smoke-test your change in `pnpm dev`, covering light/dark themes and mobile layouts. When adding logic-heavy modules, accompany them with unit tests (Vitest or Jest) under a new `__tests__/` directory and document execution steps in the PR.
